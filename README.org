...
* How to use =literate-tools=
  ...
  - Example :: Let us work with an example to illustrate the
  ...
  - Build :: Since code is situated within the narrative,
       one has to [[http://orgmode.org/worg/org-tutorials/org-publish-html-tutorial.html][build]] from these documents - the code and
       the documents - for publication.  While the code is
       understood by the compiler, the documents published
       in =html= are consumed by humans.
  ...
  - init.sh :: Since =literate-tools= is a repository on
       ...
       =literate-tools.  Please refer to [[https://github.com/vlead/vlabs-platform/blob/master/init.sh][init.sh]] that is
  ...
  - makefile :: Invoke the literate-build from this
       =makefile=.  Refer to a sample [[https://github.com/vlead/vlabs-platform/blob/master/makefile][makefile]] for
       reference. 
...
* Build Directory and .gitignore
  ...
  =docs= and =code= directory.  This build folder is not
  version controlled since it is generated.  To allow =git=
